...
export async function submitReport(data: z.infer<typeof reportSchema>, userId: string | undefined, userName: string | null | undefined, userEmail: string | null | undefined) {
  ...
  try {
    ...
    const driveResult = await createReportDriveFolder(reportRef.id, reportRef.reportNumber, reportData.locationWithin, userId, currentUser.name);
...
export async function importReports(records: any[]): Promise<{success: boolean, results: {success: boolean, reportNumber?: number, error?: string, data: any}[]}> {
    const results = [];
    const counterRef = doc(db, 'counters', 'reports');
...
export async function seedDefaultSubViolationTypes() {
...
export async function addUserByAdmin(data: z.infer<typeof adminAddUserSchema>) {
...
export async function updateUserByAdmin(data: z.infer<typeof adminUpdateUserSchema>) {
    // THIS IS A PLACEHOLDER
...
export async function exportAllData(): Promise<{ success: boolean; data?: string; error?: string }> {
...
export async function importAllData(jsonData: string): Promise<{ success: boolean; error?: string }> {
    try {
...
export async function verifyPlaceId(placeId: string): Promise<{ success: boolean; found: boolean; error?: string }> {
    if (!process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY) {
        const errorMsg = "Google Maps API key is missing on the server.";
